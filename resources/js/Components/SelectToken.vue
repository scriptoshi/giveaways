...
<template>
	<Listbox
		as="div"
		v-if="$page.props.tokens && $page.props.tokens?.length > 0"
		v-model="selected"
	>
		...
		<div class="mt-2 relative">
			...
			>
				<span class="flex items-center">
					<img
						:src="selected?.logo_uri"
						v-if="selected?.logo_uri"
						alt=""
						class="flex-shrink-0 h-6 w-6 rounded-full"
					/>
					<span class="ml-3 block font-semibold truncate"
						...
						<span
							...
							>|</span
						>
						<span class="sm:ml-3 sm:hidden">{{
							selected?.contract && shortenAddress(selected?.contract, 8)
						}}</span>
						<span class="hidden sm:inline-block">{{
							selected?.contract && shortenAddress(selected?.contract, 14)
						}}</span>
					</span>
				</span>
			...
			>
				...
				>
					...
					<ListboxOption
						as="template"
						v-for="token in $page.props.tokens"
						:key="token.id"
						:value="token"
						v-slot="{active, selected}"
					>
