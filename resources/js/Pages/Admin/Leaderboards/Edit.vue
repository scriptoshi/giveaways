...
const form = useForm({
	avatar: props.leaderboard.avatar,
	username: props.leaderboard.username,
	address: props.leaderboard.address,
	rank: props.leaderboard.rank,
	points: props.leaderboard.points,
});
const save = () => form.post(window.route("posts.edit", props.leaderboard.id));
</script>
<template>
	<Head :title="title ?? 'Edit Leaderboard'" />
	<AdminLayout>
		<template #header>
			<template v-if="title">{{ title }}</template>
			<template v-else
				>{{ $t("Edit") }} Leaderboard {{ form.name ?? crudNameSingular.id }}]</template
			>
		</template>
		<div class="bg-white">
