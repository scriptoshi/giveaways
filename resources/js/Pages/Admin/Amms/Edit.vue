...
const form = useForm(usePage().props.amm);
const chains = useChains();
form.chain = chains.value.find((c) => c.id.toString() === form.chain.chainId.toString());
const save = () => form.put(window.route("admin.amms.update", form.id));
</script>
<template>
	<AdminLayout>
		<Head :title="title ?? `Update Exchange`" />
		<main class="h-full">
				...
				<div class="flex flex-col gap-4 h-full">
					<div class="lg:flex items-center justify-between mb-4 gap-3">
						<div class="mb-4 lg:mb-0">
							<h3 class="h3">Update {{ $page.props.amm?.name }} Dex</h3>
