...
const buy = async () => {
	...
	if (parseFloat(form.amount) < props.min) {
		form.setError(
			"amount",
			t("Minimum amount is {min}", {min: `${props.min}${chain.value.nativeCurrency.symbol}`}),
		);
		return;
	}
...
<template>
	<div
		class="text-sm dark:text-gray-300 dark-text-emerald-400-text-dark p-4 rounded-sm bg-white dark:bg-gray-800"
	>
		<div class="">
			<div
				v-if="sale.loading"
			...
			<template v-else-if="sale.ended">
				<div class="flex items-center justify-between mb-4">
					<h3 class="text-base">Claim Time</h3>
			...
			<template v-else>
				<div class="flex items-center justify-between mb-4">
					<h3 class="text-base">Contribute to project fairlaunch</h3>
				...
				<div class="flex mt-2 text-xs font-semibold items-start space-x-1">
					<a
						...
						href="#"
						v-if="props.min > 0"
						@click.prevent="form.amount = props.min"
					>
						MIN
					</a>
					<a
			...
			>
				...
				<div class="flex items-center gap-1 text-right">
					<div class="break-all">
						${{ (sale.available * usdRate * 2).toFixed(4) * 1 }} ({{ sale.available }}
		...
		>
			...
			<template #content>
				<ul class="list-disc list-inside">
					<li>
						All launches on giveaways finance V2 are prize fairlaunch. It means the
						final price of the token is not known until the the sale ends
					</li>
					<li>
