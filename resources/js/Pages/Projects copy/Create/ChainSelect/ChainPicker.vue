...
const filteredChains = computed(() => {
	if (!filter.value) return chains;
	return chains.filter((f) => f.name?.toLowerCase().includes(filter.value.toLowerCase()));
});
		...
		>
			<div class="relative box-border">
				...
				>
					...
					>
						<span class="h5 m-0">{{ $t("Select A chain") }}</span>
						<button
							type="button"
							@click="close()"
					...
					<div
						class="py-[0.625] px-[0.9375rem] border border-gray-200 dark:border-gray-600 text-sm min-h-[2rem] flex content-center items-center rounded-sm"
					>
						<VueIcon
							:icon="HiSearch"
									...
									<div
										...
										:class="
											chain.id == modelValue?.id
												? 'border border-emerald-300 dark:border-emerald-400 bg-emerald-100/50 dark:bg-transparent hover:bg-emerald-100/50 '
