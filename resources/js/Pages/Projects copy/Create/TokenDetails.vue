...
const commons = ["website", "telegram", "twitter"];
const socials = [
	"discord",
	"facebook",
	"github",
...
const {t} = useI18n();
const titles = {
	website: {label: t("Website"), placeholder: t("https://projectwebsite.com")},
	...
	twitter: {
		label: t("Twitter Profile"),
		placeholder: t("https://twitter.com/yourTwitterOfficial"),
	},
	discord: {
	...
	<div class="grid gap-y-4 w-full">
		<div class="w-full max-w-lg grid">
			<label class="block mb-1 text-sm font-medium text-gray-900 dark:text-gray-300">{{
				$t("Very Very Brief Info. People hate reading...")
			}}</label>
			<ProjectDescriptionTextArea
				...
				:placeholder="
					$t(
						'Do not copy and paste from another site or use AI. Better to Write a fresh very brief description.',
					)
				"
			/>
			<p
		...
		<div class="gap-x-3 mb-3 w-full grid">
			<label class="inline-flex items-center space-x-2">
				<input
					...
					type="checkbox"
				/>
				<h3 class="text-sm">{{ $t("I need more") }}</h3>
			</label>
		</div>
		<CollapseTransition>
			<div
				v-show="showMore"
				class="gap-x-3 w-full grid md:grid-cols-3 gap-y-4"
			>
				<FormInput
					...
					:placeholder="titles[social]?.label"
					:error="errors?.[social]"
				>
					<template #trail>
