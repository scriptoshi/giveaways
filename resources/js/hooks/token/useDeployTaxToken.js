...
import { formatUnits, parseEventLogs, parseUnits, zeroAddress } from "viem";
...
export const useDeployTaxToken = (factory, form) => {
    ...
    const deploy = async () => {
        ...
        const params = [
            {
                swapAndLiquifyEnabled: form.swapAndLiquifyEnabled ?? true,
                owner: form.owner ?? address.value,
                uniswapV2Router: form.router.router,
                tokenName: form.name,
                tokenSymbol: form.symbol,
                taxFee: form.taxFee ?? 5,
                decimals: form.decimals,
                totalSupply: parseUnits(form.total_supply, form.decimals).toString(),
                liquidityFee: form.liquidityFee
            }
        ];
        status.value = t('Deploying Token ...');
        const summary = `Deploy Tax Token`;
        await call('deployContract', params, value, summary);
